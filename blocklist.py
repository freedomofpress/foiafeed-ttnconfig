...
class CustomBlocklist(BaseBlocklist):
    def check_article(self, article):
        ...
        if article.outlet in ["Miami Herald"]:
            ...
            if soup.find(attrs={"class": "byline"}) and any(
                syndication in soup.find(attrs={"class": "byline"}).get_text().lower()
                for syndication in ["ap ", "associated press", "mcclatchydc"]
            ):
                blocked = True
    ...
    def check_paragraph(self, article, paragraph):
        ...
        if article.outlet == "404 Media":
            # Jason Leopold's bio affirms their love for FOIA, resulting in false positives
